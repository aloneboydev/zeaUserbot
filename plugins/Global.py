...
from Amang.dB.gban_mute_db import (
    gban,
    gmute,
    is_gbanned,
    is_gmuted,
...
from Amang.dB.gcast_blacklist_db import (
    add_gblacklist,
    is_gblacklisted,
    rem_gblacklist,
)
from Amang.fns.tools import create_tl_btn, format_btn, get_msg_button
...
@amang_cmd(pattern="gpromote( (.*)|$)", fullsudo=True)
async def _(e):
...
@amang_cmd(pattern="gdemote( (.*)|$)", fullsudo=True)
async def _(e):
    x = e.pattern_match.group(1).strip()
    amang_bot = e.client
    if not x:
    ...
    if user:
        ...
        if user.isdigit():
            user = int(user)
        try:
            name = await amang_bot.get_entity(user)
        except BaseException:
            return await e.eor(f"`Tidak Ada Pengguna Ditemukan Terkait {user}`", time=5)
        ...
        rank = "Not AdMin"
        c = 0
        async for x in amang_bot.iter_dialogs():
            if (
                "group" in key.lower()
                and x.is_group
                or "group" not in key.lower()
...
@amang_cmd(pattern="g(admin|)cast( (.*)|$)", fullsudo=False)
async def gcast(event):
    ...
    if err != "":
        ...
        text += f"\Anda dapat melakukan `{HNDLR}amang gcast-error.log` untuk mengetahui laporan kesalahan."
    await kk.edit(text)


@amang_cmd(pattern="gucast( (.*)|$)", fullsudo=True)
...
@amang_cmd(pattern="gkick( (.*)|$)", fullsudo=True)
async def gkick(e):
...
@amang_cmd(pattern="gmute( (.*)|$)", fullsudo=True)
async def _(e):
...
@amang_cmd(pattern="ungmute( (.*)|$)", fullsudo=True)
async def _(e):
...
async def list_gengbanned(event):
    ...
    if len(gbanned_users) > 4096:
        ...
        await x.reply(
            file="gbanned.txt",
            message=f"List of users GBanned by {inline_mention(amang_bot.me)}",
        )
...
@amang_cmd(pattern="gblacklist$")
async def blacklist_(event):
    await gblacker(event, "add")


@amang_cmd(pattern="ungblacklist$")
