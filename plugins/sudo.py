...
from Amang._misc import sudoers

from . import get_string, inline_mention, udB, amang_bot, amang_cmd


@amang_cmd(pattern="addsudo( (.*)|$)", fullsudo=True)
async def _(ult):
...
@amang_cmd(pattern="delsudo( (.*)|$)", fullsudo=True)
async def _(ult):
    inputs = ult.pattern_match.group(1).strip()
    if ult.reply_to_msg_id:
        replied_to = await ult.get_reply_message()
...
async def _(ult):
    ...
    if not m:
        m = "[False](https://graph.org/AmangUserbot-01-21)"
