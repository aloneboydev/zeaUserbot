...
from Amang.fns.admins import ban_time
...
async def prmte(amang):
    xx = await amang.eor(get_string("com_1"))
    user, rank = await get_uinfo(amang)
    rank = rank or "Admin"
    FullRight = False
    if not user:
    ...
    try:
        ...
        await eod(
            xx, get_string("pro_2").format(inline_mention(user), amang.chat.title, rank)
        )
...
async def dmote(amang):
    ...
    try:
        ...
        await eod(xx, get_string("de_2").format(inline_mention(user), amang.chat.title))
    except Exception as ex:
        return await xx.edit(f"`{ex}`")
...
async def bban(amang):
    something = await get_uinfo(amang)
    if not something:
        return
    user, reason = something
    if not user:
        return await eod(amang, get_string("ban_1"))
    ...
    try:
        ...
        return await eod(amang, get_string("ban_3"))
    senderme = inline_mention(await amang.get_sender())
    userme = inline_mention(user)
    text = get_string("ban_4").format(userme, senderme, amang.chat.title)
    if reason:
        text += get_string("ban_5").format(reason)
    await eod(amang, text)
...
async def uunban(amang):
    xx = await amang.eor(get_string("com_1"))
    if amang.text[1:].startswith("unbanall"):
        return
    something = await get_uinfo(amang)
    if not something:
        return
    ...
    sender = inline_mention(await amang.get_sender())
    text = get_string("unban_3").format(inline_mention(user), sender, amang.chat.title)
    if reason:
        text += get_string("ban_5").format(reason)
    await xx.edit(text)
...
async def kck(amang):
    if "kickme" in amang.text:
        return
    if amang.is_private:
        return await amang.eor("`Gunakan ini di Grup/Saluran.`", time=5)
    ml = amang.text.split(" ", maxsplit=1)[0]
    xx = await amang.eor(get_string("com_1"))
    something = await get_uinfo(amang)
    if not something:
        return
...
@amang_cmd(pattern="pin$", manager=True, require="pin_messages", fullsudo=True)
async def pin(msg):
    if not msg.is_reply:
...
async def unp(amang):
    xx = await amang.eor(get_string("com_1"))
    ch = (amang.pattern_match.group(1).strip()).strip()
    msg = None
    if amang.is_reply:
        msg = amang.reply_to_msg_id
    elif ch != "all":
...
async def pin_message(amang):
    match = amang.pattern_match.group(1).strip()
    if not amang.is_reply:
        return await amang.eor("`Balas pesan..`", time=6)
    if not match:
        return await amang.eor("`Harap sediakan waktu..`", time=8)
    msg = await amang.eor(get_string("com_1"))
...
@amang_cmd(pattern="purge( (.*)|$)", manager=True, require="delete_messages")
...
@amang_cmd(pattern="pinned", manager=True, groups_only=True)
...
async def autodelte(amang):
    match = amang.pattern_match.group(1).strip()
    if not match or match not in ["24h", "7d", "1m", "off"]:
        return await amang.eor("`Silakan Gunakan dalam Format yang Tepat..`", time=5)
    if match == "24h":
        tt = 3600 * 24
    elif match == "7d":
        tt = 3600 * 24 * 7
    ...
    await amang.eor(f"Status Hapus Otomatis Diubah menjadi `{match}` !")
