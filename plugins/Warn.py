...
# <https://www.github.com/amangtele/AmangUserbotgUserbotgUserbotgUserbotgUserbotgUserbotgUserbotgUserbotgUserbotgUserbotgUserbotgUserbotgUserbotgUserbotgUserbotgUserbotgUserbotgUserbotgUserbot/blob/main/LICENSE/>.
"""
◈ Perintah Tersedia
...
from Amang.dB.warn_db import add_warn, reset_warn, warns

from . import eor, get_string, inline_mention, udB, amang_cmd
...
async def warn(e):
    ...
    if reply:
        ...
        try:
            user = e.text.split()[1]
            if user.startswith("@"):
                ok = await amang_bot.get_entity(user)
                user = ok.id
            else:
                user = int(user)
    ...
    if count + 1 >= number:
        if "ban" in action:
            try:
                await amang_bot.edit_permissions(e.chat_id, user, view_messages=False)
            except BaseException:
                return await e.eor("`Ada yang salah.`", time=5)
        elif "kick" in action:
            try:
                await amang_bot.kick_participant(e.chat_id, user)
            except BaseException:
                return await e.eor("`Ada yang salah.`", time=5)
        elif "mute" in action:
            try:
