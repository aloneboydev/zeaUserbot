...
from . import bash, get_string, mediainfo, amang_cmd


@amang_cmd(pattern="glitch$")
async def _(e):
    try:
    ...
    cmd = f"glitch_me gif --line_count 200 -f 10 -d 50 '{ok}' amang.gif"
    stdout, stderr = await bash(cmd)
    await e.reply(file="amang.gif", force_document=False)
    await xx.delete()
    os.remove(ok)
    os.remove("amang.gif")
