...
from Amang.dB.notes_db import add_note, get_notes, list_note, rem_note
from Amang.fns.tools import create_tl_btn, format_btn, get_msg_button

from . import events, get_string, mediainfo, udB, amang_bot, amang_cmd
from ._inline import something


@amang_cmd(pattern="addnote( (.*)|$)", admins_only=True)
async def an(e):
    ...
    amang_bot.add_handler(notes, events.NewMessage())


@amang_cmd(pattern="remnote( (.*)|$)", admins_only=True)
async def rn(e):
    wrd = (e.pattern_match.group(1).strip()).lower()
    chat = e.chat_id
    if not wrd:
...
@amang_cmd(pattern="listnote$", admins_only=True)
async def lsnote(e):
