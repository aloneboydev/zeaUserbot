...
from Amang.dB.logusers_db import *
from Amang.dB.pmpermit_db import *
...
if udB.get_key("PMSETTING"):
    if udB.get_key("AUTOAPPROVE"):
        ...
        async def autoappr(e):
            ...
            try:
                await amang_bot.edit_folder(miss.id, folder=0)
    ...
    async def permitpm(event):
        ...
        if not is_approved(user.id) and event.text != UND:
            if Redis("MOVE_ARCHIVE"):
                try:
                    await amang_bot.edit_folder(user.id, folder=1)
            ...
            if user.id in LASTMSG:
                ...
                if event.text != prevmsg:
                    ...
                    if inline_pm:
                        results = await amang_bot.inline_query(
                        ...
                        _to_delete[user.id] = await amang_bot.send_file(
                            user.id,
                            PMPIC,
                            caption=message_,
                        )
                    else:
                        _to_delete[user.id] = await amang_bot.send_message(
                            user.id, message_
                        )
            ...
            if COUNT_PM[user.id] >= WARNS:
                ...
                await amang_bot(BlockRequest(user.id))
                await amang_bot(ReportSpamRequest(peer=user.id))
                await asst.edit_message(
                    int(udB.get_key("LOG_CHANNEL")),
                    _not_approved[user.id],
    ...
    @amang_cmd(pattern="(a|ok)(?: |$)", fullsudo=True)
    ...
    @amang_cmd(pattern="(no|tolak)(?: |$)", fullsudo=True)
...
async def list_approved(event):
    ...
    for i in all:
        try:
            name = get_display_name(await amang_bot.get_entity(i))
        except BaseException:
            name = ""
...
async def apr_in(event):
    ...
    if not is_approved(uid):
        approve_user(uid)
        try:
            await amang_bot.edit_folder(uid, folder=0)
        except BaseException:
            pass
        try:
            user = await amang_bot.get_entity(uid)
        except BaseException:
...
async def blck_in(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    try:
        await amang_bot(BlockRequest(uid))
    except BaseException:
        pass
    try:
        user = await amang_bot.get_entity(uid)
...
async def unblck_in(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    try:
        await amang_bot(UnblockRequest(uid))
    except BaseException:
        pass
    try:
        user = await amang_bot.get_entity(uid)
...
async def ytfuxist(e):
    try:
        ...
        await amang_bot.delete_messages(e.chat_id, e.id)
...
async def in_pm_ans(event):
        ...
        if res:
            res = [
                await event.builder.document(
                    ...
                    description="[↻ꝛɪᴢ](https://t.me/amwang)",
                    text=msg_,
                    buttons=buttons,
                    link_preview=False,
                )
    ...
    await event.answer(res, switch_pm="• Amang •", switch_pm_param="start")


@callback(re.compile("admin_only(.*)"), from_users=[amang_bot.uid])
