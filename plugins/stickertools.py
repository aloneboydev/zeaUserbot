...
async def hehe(args):
    ...
    if message.photo:
        photo = io.BytesIO()
        photo = await amang_bot.download_media(message.photo, photo)
    elif message.file and "image" in message.file.mime_type.split("/"):
        photo = io.BytesIO()
        await amang_bot.download_file(message.media.document, photo)
        if (
    ...
    if photo:
        ...
        if (
            ...
            async with amang_bot.conversation("@Stickers") as conv:
                try:
                    await conv.send_message("/addsticker")
                except YouBlockedUserError:
                    LOGS.info("Unblocking @Stickers for kang...")
                    await amang_bot(functions.contacts.UnblockRequest("stickers"))
                    await conv.send_message("/addsticker")
                ...
                await conv.get_response()
                await amang_bot.send_read_acknowledge(conv.chat_id)
        else:
...
async def ultdestroy(event):
    ...
    await event.client.download_media(ult, "amang.tgs")
    xx = await event.eor(get_string("com_1"))
    await bash("lottie_convert.py amang.tgs json.json")
    ...
    file = await con.animated_sticker("json.json", "amang.tgs")
    if file:
        await event.client.send_file(
            event.chat_id,
            file="amang.tgs",
