...
from Amang import _ignore_eval
...
try:
    ...
    from Amang.fns.tools import safe_load
try:
...
@amang_cmd(pattern="bash", fullsudo=False, only_devs=False)
async def _(event):
    ...
    if len(OUT) > 4096:
        amang = err + out
        with BytesIO(str.encode(amang)) as out_file:
...
@amang_cmd(pattern="eval", fullsudo=False, only_devs=False)
async def _(event):
    try:
        cmd = event.text.split(maxsplit=1)[1]
    except IndexError:
    ...
    if any(item in cmd for item in KEEP_SAFE().All) and (
        not (event.out or event.sender_id == amang_bot.uid)
    ):
...
@amang_cmd(pattern="cpp", only_devs=False)
async def doie(e):
    ...
    open("cpp-amang.cpp", "w").write(match)
    m = await bash("g++ -o CppAmang cpp-amang.cpp")
    o_cpp = f"• **Eval-Cpp**\n`{match}`"
    if m[1]:
        o_cpp += f"\n\n**• Error :**\n`{m[1]}`"
        if len(o_cpp) > 3000:
            os.remove("cpp-amang.cpp")
            if os.path.exists("CppAmang"):
                os.remove("CppAmang")
