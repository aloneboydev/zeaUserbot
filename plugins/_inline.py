...
from Amang._misc._assistant import callback, in_pattern
from Amang.dB._core import HELP, LIST
from Amang.fns.helper import gen_chlog, time_formatter, updater
from Amang.fns.misc import split_list
...
async def inline_handler(event):
    ...
    if inline_pic():
        ...
        result = await event.builder.article(
            title="Amang Help Menu", text=text, buttons=_main_help_menu
        )
...
async def help_func(amang):
    key, count = amang.data_match.group(1).decode("utf-8").split("_")
    if key == "VCBot" and HELP.get("VCBot") is None:
        return await amang.answer(get_string("help_12"), alert=True)
    elif key == "Addons" and HELP.get("Addons") is None:
        return await amang.answer(get_string("help_13").format(HNDLR), alert=True)
    if "|" in count:
        _, count = count.split("|")
    count = int(count) if count else 0
    text = _strings.get(key, "").format(riz, len(HELP.get(key)))
    await amang.edit(text, buttons=page_num(count, key), link_preview=False)
...
async def _(event):
    ...
    if len(changelog_str) > 1024:
        ...
        with open("amang_updates.txt", "w+") as file:
            file.write(tl_chnglog)
        await event.edit(
            get_string("upd_5"),
            file="amang_updates.txt",
            buttons=[
                [Button.inline("• Uᴘᴅᴀᴛᴇ Nᴏᴡ •", data="updatenow")],
                [Button.inline("«", data="ownr")],
            ],
        )
        remove("amang_updates.txt")
    else:
        await event.edit(
            changelog_str,
            buttons=[
...
async def ibuild(e):
    ...
    if pic:
        try:
            ...
            LOGS.exception(er)
    result = [
        await builder.article("Amang Op", text=txt, link_preview=False, buttons=btn)
    ]
