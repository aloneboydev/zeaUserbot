...
async def autoupdate_local_database():
    from .. import asst, udB, amang_bot, Var

    global db_url
    db_url = (
        udB.get_key("TGDB_URL") or Var.TGDB_URL or amang_bot._cache.get("TGDB_URL")
    )
    if db_url:
        _split = db_url.split("/")
        _channel = _split[-2]
...
async def autobot():
    from .. import udB, amang_bot

    if udB.get_key("BOT_TOKEN"):
        return
    await amang_bot.start()
    ...
    if who.username:
        ...
        username = "amang_" + (str(who.id))[5:] + "_bot"
    bf = "@BotFather"
    await amang_bot(UnblockRequest(bf))
    await amang_bot.send_message(bf, "/cancel")
    await asyncio.sleep(1)
    await amang_bot.send_message(bf, "/newbot")
    await asyncio.sleep(1)
    isdone = (await amang_bot.get_messages(bf, limit=1))[0].text
    if isdone.startswith("That I cannot do.") or "20 bots" in isdone:
        ...
        sys.exit(1)
    await amang_bot.send_message(bf, name)
    await asyncio.sleep(1)
    isdone = (await amang_bot.get_messages(bf, limit=1))[0].text
    if not isdone.startswith("Good."):
        await amang_bot.send_message(bf, "My Assistant Bot")
        await asyncio.sleep(1)
        isdone = (await amang_bot.get_messages(bf, limit=1))[0].text
        if not isdone.startswith("Good."):
            LOGS.critical(
            ...
            sys.exit(1)
    await amang_bot.send_message(bf, username)
    await asyncio.sleep(1)
    isdone = (await amang_bot.get_messages(bf, limit=1))[0].text
    await amang_bot.send_read_acknowledge("botfather")
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = "amang_" + (str(who.id))[6:] + str(ran) + "_bot"
        await amang_bot.send_message(bf, username)
        await asyncio.sleep(1)
        isdone = (await amang_bot.get_messages(bf, limit=1))[0].text
    if isdone.startswith("Done!"):
        ...
        await enable_inline(amang_bot, username)
        LOGS.info(
...
async def autopilot():
    from .. import asst, udB, amang_bot

    channel = udB.get_key("LOG_CHANNEL")
    new_channel = None
    if channel:
        try:
            chat = await amang_bot.get_entity(channel)
    ...
    if not channel:
        ...
        async def _save(exc):
            udB._cache["LOG_CHANNEL"] = amang_bot.me.id
            await asst.send_message(
                amang_bot.me.id, f"Gagal Membuat Saluran Log karena {exc}.."
            )
    ...
    assistant = True
    try:
        await amang_bot.get_permissions(int(channel), asst.me.username)
    except UserNotParticipantError:
        try:
            await amang_bot(InviteToChannelRequest(int(channel), [asst.me.username]))
        except BaseException as er:
    ...
    if isinstance(chat.photo, ChatPhotoEmpty):
        photo = await download_file(
            "https://telegra.ph/file/4fe47bc238c18b11f8258.jpg", "logo.jpg"
        )
        ll = await amang_bot.upload_file(photo)
        try:
            await amang_bot(
...
async def customize():
    ...
    try:
        ...
        if not amang_bot.me.username:
            sir = amang_bot.me.first_name
        else:
            sir = f"@{amang_bot.me.username}"
        file = random.choice(
            [
        ...
        await asyncio.sleep(1)
        await amang_bot.send_message("botfather", "/cancel")
        await asyncio.sleep(1)
        await amang_bot.send_message("botfather", "/setuserpic")
        await asyncio.sleep(1)
        isdone = (await amang_bot.get_messages("botfather", limit=1))[0].text
        if isdone.startswith("Invalid bot"):
            LOGS.info("Error while trying to customise assistant, skipping...")
            return
        await amang_bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await amang_bot.send_file("botfather", file)
        await asyncio.sleep(2)
        await amang_bot.send_message("botfather", "/setabouttext")
        await asyncio.sleep(1)
        await amang_bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await amang_bot.send_message(
            "botfather", f"✨ Hello ✨!! I'm Assistant Bot of {sir}"
        )
        await asyncio.sleep(2)
        await amang_bot.send_message("botfather", "/setdescription")
        await asyncio.sleep(1)
        await amang_bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await amang_bot.send_message(
            "botfather",
            f"✨ Powerful Amang Assistant Bot ✨\n✨ Master ~ {sir} ✨\n\n✨ Powered By ~ @purapuranyagcsupport ✨",
        )
        await asyncio.sleep(2)
...
async def plug(plugin_channels):
    ...
    for chat in plugin_channels:
        LOGS.info(f"{'•'*4} {chat}")
        try:
            async for x in amang_bot.iter_messages(
                ...
                try:
                    ...
                    LOGS.info(f"Amang - PLUGIN_CHANNEL - ERROR - {plugin}")
                    LOGS.exception(e)
                    os.remove(plugin)
        except Exception as er:
            LOGS.exception(er)
...
• Here, are the Some Basic stuff from, where you can Know, about its Usage."""
        PHOTO = "https://telegra.ph/file/4fe47bc238c18b11f8258.jpg"
        BTTS = Button.inline("• Click to Start •", "initft_2")
        udB.set_key("INIT_DEPLOY", "Done")
    else:
        MSG = f"**Amang has been deployed!**\n➖➖➖➖➖➖➖➖➖➖\n**UserMode**: {inline_mention(amang_bot.me)}\n**Assistant**: @{asst.me.username}\n➖➖➖➖➖➖➖➖➖➖\n**Support**: @purapuranyagcsupport\n➖➖➖➖➖➖➖➖➖➖"
        BTTS, PHOTO = None, None
        prev_spam = udB.get_key("LAST_UPDATE_LOG_SPAM")
        if prev_spam:
            try:
                await amang_bot.delete_messages(chat_id, int(prev_spam))
            except Exception as E:
    ...
    try:
        ...
        try:
            await (await amang_bot.send_message(chat_id, str(e))).delete()
            spam_sent = await asst.send_message(chat_id, MSG, file=PHOTO, buttons=BTTS)
        ...
        LOGS.info(el)
        try:
            spam_sent = await amang_bot.send_message(chat_id, MSG)
...
async def WasItRestart(udb):
    ...
    if not key:
        return
    from .. import asst, amang_bot

    try:
        data = key.split("_")
        who = asst if data[0] == "bot" else amang_bot
...
async def enable_inline(amang_bot, username):
    bf = "BotFather"
    await amang_bot.send_message(bf, "/setinline")
    await asyncio.sleep(1)
    await amang_bot.send_message(bf, f"@{username}")
    await asyncio.sleep(1)
    await amang_bot.send_message(bf, "Search")
    await amang_bot.send_read_acknowledge(bf)
