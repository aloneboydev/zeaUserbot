...
def load_addons(plugin_name):
    ...
    if base_name.startswith("__"):
        return
    from .. import HNDLR, LOGS, asst, udB, amang_bot
    from .._misc import _supporter as xxx
    from Amang import fns
    from .._misc._assistant import asst_cmd, callback, in_pattern
    from .._misc._decorators import amang_cmd
    ...
    mod.asst = asst
    mod.tgbot = asst
    mod.amang_bot = amang_bot
    mod.ub = amang_bot
    mod.bot = amang_bot
    mod.amang = amang_bot
    mod.borg = amang_bot
    mod.telebot = amang_bot
    mod.jarvis = amang_bot
    mod.friday = amang_bot
    mod.eod = eod
    mod.edit_delete = eod
    mod.LOGS = LOGS
