...
from Amang.exceptions import DependencyMissingError
...
async def randomchannel(
    tochat, channel, range1, range2, caption=None, client=amang_bot
):
...
async def allcmds(event, telegraph):
    ...
    for z in LIST.keys():
        ...
        for zz in LIST[z]:
            txt += HNDLR + zz + "\n"
        txt += "\n\n"
    t = telegraph.create_page(title="Amang All Cmds", content=[txt])
    await eor(event, f"All Amang Cmds : [Click Here]({t['url']})", link_preview=False)
...
async def ReTrieveFile(input_file_name):
    ...
    async with aiohttp.ClientSession() as ses:
        async with ses.post(
            ...
            name = check_filename("amang-rmbg.png")
...
async def _insta_login():
    if "insta_creds" in amang_bot._cache:
        return amang_bot._cache["insta_creds"]
    ...
    if username and password:
        ...
        try:
            ...
            amang_bot._cache.update({"insta_creds": cl})
...
async def get_insta_code(username, choice):
    ...
    async with asst.conversation(amang_bot.uid, timeout=60 * 2) as conv:
