...
if run_as_module:
    ...
    async def safeinstall(event):
        ...
        if plug in HELP:
            ...
            for i in HELP[plug]:
                output += i
            output += "©↻ꝛɪᴢ"
            await eod(ok, f"✓ `Amang - Installed`: `{plug}` ✓\n\n{output}")
    ...
    async def heroku_logs(event):
        ...
        ok = app.get_log()
        with open("amang-heroku.log", "w") as log:
            log.write(ok)
        await event.client.send_file(
            event.chat_id,
            file="amang-heroku.log",
            thumb=AyConfig.thumb,
            caption="**Amang Heroku Logs.**",
        )

        os.remove("amang-heroku.log")
        await xx.delete()
    ...
    def gen_chlog(repo, diff):
        ...
        ch_log = tldr_log = ""
        ch = f"<b>Amang {amang_version} updates for <a href={UPSTREAM_REPO_URL}/tree/{ac_br}>[{ac_br}]</a>:</b>"
        ch_tl = f"Amang {amang_version} updates for {ac_br}:"
        d_form = "%d/%m/%y || %H:%M"
