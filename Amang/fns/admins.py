...
async def _callback_check(event):
    ...
    if not _amang_cache.get("admin_callback"):
        _amang_cache.update({"admin_callback": {id_: None}})
    else:
        _amang_cache["admin_callback"].update({id_: None})
    while not _amang_cache["admin_callback"].get(id_):
        await asyncio.sleep(1)
    key = _amang_cache.get("admin_callback", {}).get(id_)
    del _amang_cache["admin_callback"][id_]
    return key


async def get_update_linked_chat(event):
    if _amang_cache.get("LINKED_CHATS") and_amang_cachee["LINKED_CHATS"].get(event.chat_id):
        ...
        if _amang_cache.get("LINKED_CHATS"):
            _amang_cache["LINKED_CHATS"].update({event.chat_id: {"linked_chat": _ignore}})
        else:
            _amang_cache.update(
