...
def telegraph_client():
    ...
    gd_name = amang_bot.full_name
    short_name = gd_name[:30]
    profile_url = (
        f"https://t.me/{amang_bot.me.username}"
        if amang_bot.me.username
        else "https://t.me/purapuranyagcsupport"
    )
    try:
        ...
        if "SHORT_NAME_TOO_LONG" in str(er):
            TelegraphClient.create_account(
                short_name="amanguser", author_name=gd_name, author_url=profile_url
            )
        else:
            LOGS.exception(er)
            return
...
async def Carbon(
    ...
    **kwargs,
    ...
    kwargs["title"] = kwargs.get("title", "Amang")
    # else:
    #    kwargs["code"] = code
