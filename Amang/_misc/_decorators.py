...
from ..version import amang_version as amang_ver
...
def amang_cmd(
    pattern=None, manager=False, amang_bot=amang_bot, asst=asst, **kwargs
):
    ...
    def decor(dec):
        async def wrapp(ay):
            ...
            try:
                ...
                await amang_bot.disconnect()
                await asyncio.sleep(fwerr.seconds + 10)
                await amang_bot.connect()
                await asst.send_message(
                    udB.get_key("LOG_CHANNEL"),
                ...
                await asst.send_message(
                    ...
                    buttons=[
                        ...
                        Button.url(
                            "Repl",
                            "https://replit.com/@ErichDaniken/Generate-Telegram-String-Session",
                        ),
                    ],
                )
                sys.exit()
                ...
                ftext = "**Amang Client Error:** `Forward this to` @purapuranyagcsupport\n\n"
                ftext += "**Amang Version:** `" + str(pyver)
                ftext += "`\n**Userbot Version:** `" + str(amang_ver)
                ftext += "`\n**Telethon Version:** `" + str(telever)
                ftext += f"`\n**Hosted At:** `{HOSTED_ON}`\n\n"
                ftext += "--------START AMANG CRASH LOG--------"
                ftext += "\n**Date:** `" + date
                ...
                if len(ftext) > 4096:
                    with BytesIO(ftext.encode()) as file:
                        ...
                        error_log = await asst.send_file(
                            udB.get_key("LOG_CHANNEL"),
                            file,
                            caption="**Amang Client Error:** `Forward this to` @purapuranyagcsupport\n\n",
                        )
                else:
