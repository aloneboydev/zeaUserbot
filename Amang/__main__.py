...
def main():
    ...
    if (
        ...
        and amang_bot.run_in_loop(updater())
    ):
        amang_bot.run_in_loop(bash("bash installer.sh"))

        os.execl(sys.executable, "python3", "-m", "Amang")

    amang_bot.run_in_loop(startup_stuff())

    amang_bot.me.phone = None

    if not amang_bot.me.bot:
        udB.set_key("OWNER_ID", amang_bot.uid)

    LOGS.info("Initialising...")

    amang_bot.run_in_loop(autopilot())
    ...
    # Customize amang Assistant...
    amang_bot.run_in_loop(customize())

    # Load Addons from Plugin Channels.
    if plugin_channels:
        amang_bot.run_in_loop(plug(plugin_channels))

    # Send/Ignore Deploy Message..
    if not udB.get_key("LOG_OFF"):
        amang_bot.run_in_loop(ready())

    # Edit Restarting Message (if It's restarting)
    amang_bot.run_in_loop(WasItRestart(udB))
    ...
    LOGS.info(
        f"Took {time_formatter((time.time() - start_time)*1000)} to start ◈ AmangUserbot◈"
    )
    LOGS.info(suc_msg)
