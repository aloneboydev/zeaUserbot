...
from Amang._misc import SUDO_M, owner_and_sudos
from Amang.dB.asst_fns import *
...
async def amang(event):
    ...
    if event.sender_id not in SUDO_M.fullsudos:
        ok = ""
        me = inline_mention(amang_bot.me)
        ...
        if not udB.get_key("STARTMSG"):
            ...
            await event.reply(
                f"Hey there {mention}, this is Amang Assistant of {me}!\n\n{ok}",
