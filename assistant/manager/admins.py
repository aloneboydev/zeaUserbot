...
@amang_cmd(pattern="d(kick|ban)", manager=True, require="ban_users")
async def dowj(e):
...
@callback(re.compile("cc_(.*)"), func=_amang_cache.get("admin_callback"))
async def callback_(event):
    data = event.data_match.group(1).decode("utf-8")
    if data not in _amang_cache.get("admin_callback", {}):
        return
    try:
    ...
    _amang_cache["admin_callback"].update({data: (event.sender, perm)})
