...
from Amang.fns.misc import google_search
from Amang.fns.tools import (
    _webupload_cache,
    async_searcher,
    get_ofox,
    saavn_search,
...
aypic = "https://telegra.ph/file/4fe47bc238c18b11f8258.jpg"
...
async def repo(e):
    ...
    await e.answer(res, switch_pm="Amang Repo.", switch_pm_param="start")
...
async def twitter_search(event):
    ...
    if _amang_cache.get("twitter"):
        _amang_cache["twitter"].update({match: reso})
    else:
        _amang_cache.update({"twitter": {match: reso}})
        ...
        return await ay.answer(
            [
                await ay.builder.article(
                    title="How to Use?",
                    description="Tl Searcher by Amang",
...
InlinePlugin.update(
    {
        ...
        "Sᴇᴀʀᴄʜ Oɴ Gᴏᴏɢʟᴇ": "go purapuranyagcsupport",
        "WʜɪSᴘᴇʀ": "wspr @username Hello🎉",
        "YᴏᴜTᴜʙᴇ Dᴏᴡɴʟᴏᴀᴅᴇʀ": "yt Ed Sheeran Perfect",
        "Piston Eval": "run javascript console.log('Hello Amang')",
        "OʀᴀɴɢᴇFᴏx🦊": "ofox beryllium",
        "Tᴡɪᴛᴛᴇʀ Usᴇʀ": "twitter purapuranyagcsupport",
