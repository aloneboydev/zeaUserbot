...
try:
    from Amang.fns.gDrive import GDriveManager
except ImportError:
    GDriveManager = None
...
from Amang.fns.helper import fast_download, progress
from Amang.fns.tools import Carbon, async_searcher, get_paste, telegraph_client
from Amang.startup.loader import Loader
...
_buttons = {
    "otvars": {
        "text": "Variabel lain untuk ditetapkan @purapuranyagcsupport:",
        "buttons": [
            [
...
async def update(eve):
    ...
    if heroku_api:
        ...
        execl(sys.executable, sys.executable, "-m", "Amang")
...
async def _(e):
    ...
    async with asst.conversation(e.sender_id) as conv:
        ...
        if GDrive._create_token_file(code=code.text):
            await conv.send_message(
                "`Berhasil!\nAnda siap menggunakan Google Drive Amang Userbot.`",
